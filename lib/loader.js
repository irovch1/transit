 ...
 */
function load(filePath, layerId, agencyId, agencyName, layerName) {
  var stream = null;
  try {
  ...
  if(layerId === 'stops') {
    ...
    stream = fare.loadFareOutlets(filePath, agencyId, agencyName, layerName);
  } else {
...
function loadTransitFeeds(transitConfig) {
  ...
  transitConfig.feeds.forEach(function(feed) {
    ...
    var stream = load(filePath, feed.layerId, feed.agencyId, feed.agencyName, feed.layerName);
    if(stream){
